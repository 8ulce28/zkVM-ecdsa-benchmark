...
run valida in an x86_64 emulated environment with `glibc-2.39`. Last but not least, Valida is under development; so, currently, it isn’t fully zero-knowledge because inputs are not cryptographically hidden.
