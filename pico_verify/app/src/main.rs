...
pub fn main() {
    ...
    let message_hash: [u8; 32] = [127, 208, 178, 212, 164, 94, 120, 175, 39, 225, 46, 45, 227, 107, 13, 145,
    174, 219, 233, 37, 221, 212, 183, 86, 115, 138, 76, 85, 204, 251, 36, 114];
    ...
    let pk = PublicKey::from_slice(&pk_bytes)
        .expect("Public key cannot be created from bytes");
    ...
    let signature = Secp256k1RecoverableSignature::from_compact(&sig_bytes, recovery_id).unwrap();

    let public_key = secp.recover_ecdsa(&message, &signature)
        .expect("Public key cannot be recovered");
    ...
    assert_eq!(public_key, pk);  // Checking the equivalence of expected and extracted pk
}
